...
The functions are significantly based on the reduction scripts developed
by the DSHARP (Andrews+2018) and MAPS (Czekala+2021) teams 
...
def LSRKvel_to_chan(msfile, field, spw, restfreq, LSRKvelocity):
    ...
    data_desc_id = np.where(spw_col==spw)[0]
    tb.close()

    tb.open(msfile)
    data_desc_col = tb.getcol('DATA_DESC_ID')
    obs_col = tb.getcol('OBSERVATION_ID')
    tb.close()

    obsid = int(np.unique(obs_col[np.where(data_desc_col==data_desc_id)]))
...
def avg_cont(ms_dict, output_prefix, flagchannels = '', maxchanwidth = 250.,
             datacolumn = 'data', contspws = None, width_array = None, timebin=None):
    """
...
def tclean_wrapper(vis, imagename, deconvolver='hogbom', gridder = 'standard', scales=[0,10,20,40], smallscalebias = 0.6, mask = '', threshold = '0.2mJy', imsize = None, cellsize = None, interactive = False, robust = 0.5, gain = 0.3, niter = 50000, cycleniter = 300, uvtaper = [], savemodel = 'none', datacolumn='data', parallel=False, spw=None):

    """
    Wrapper for tclean with keywords set to values desired for the Large Program imaging
    See the CASA 6.2.1 documentation for tclean to get the definitions of all the parameters
    """
    if imsize is None:
    ...
    if deconvolver == 'multiscale':
        ...
        print("Error: set deconvolver to multiscale or hogbom")

def image_each_obs(mpi,ms_dict, prefix, scales, smallscalebias = 0.6, mask = '', threshold = '0.2mJy', imsize = None, cellsize = None, interactive = False, robust = 0.5, gain = 0.3, niter = 50000, cycleniter = 300):
    """
    ...
    """
    if mpi:
        msfile = prefix+'_'+ms_dict['name']+'_initcont.mms'
    else:
    ...
    imagename = prefix+'_'+ms_dict['name']+'_initcont'
    for ext in ['.image', '.mask', '.model', '.pb', '.psf', '.residual', '.sumwt']:
        os.system('rm -rf '+ imagename + ext)
    tclean(vis= msfile,
           imagename = imagename,
           ...
           parallel = True,
           nterms = 1)
    print("Image saved in %s.image" % (prefix+'_'+ms_dict['name']+'_initcont',))
...
def split_all_obs(msfile, nametemplate):
    ...
    Split out individual observations in a measurement set (this assumes that the ms only contains a single source!!)
    ...
    #yes, it would be more logical to split out by observation id, but splitting
    #out by observation id in practice leads to some issues with the metadata
    for i in obs_ids:
        data_desc_ids = np.unique(data_desc_col[np.where(obs_col==i)])
        spws = spw_col[data_desc_ids]

        #sometimes the MS secretly has multiple field IDs lurking even if listobs
        #only shows one field
        fields = np.unique(field_col[np.where(obs_col==i)])
        if len(spws)==1:
            spwstring = str(spws[0])
        else:
            spwstring = ",".join([str(i) for i in spws])

        if len(fields)==1:
            fieldstring = str(fields[0])
        else:
            fieldstring = ",".join([str(i) for i in fields])
        #start of CASA commands
...
def export_MS(msfile):
    ...
    if filename[-4:]=='.mms':
        ...
        print("MS label must end in '.mms' or '.ms'")
        return
    # strip off the '.ms'
    MS_filename = filename.replace(exten, '')

    # get information about spectral windows

    tb.open(MS_filename+exten+'/SPECTRAL_WINDOW')
    num_chan = tb.getcol('NUM_CHAN').tolist()
    tb.close()

    # spectral averaging (1 channel per SPW)

    os.system('rm -rf %s' % MS_filename+'_spavg'+exten)
    split(vis=MS_filename+exten, width=np.max(num_chan), datacolumn='data',
    outputvis=MS_filename+'_spavg'+exten, keepflags=False)

    # get the data tables
    tb.open(MS_filename+'_spavg'+exten)
    ...
    tb.open(MS_filename+'_spavg'+exten+'/SPECTRAL_WINDOW')
...
def plot_deprojected(filelist, incl = 0, PA = 0, offx = 0, offy = 0, fluxscale = None,
                     uvbins = None, uvmax=1000, show_err = True, plot_label = None, show=False):
    """
    ...
    plot_label: path to save output figure to
    """
    ...
    if ((allminvis < 0) or (allminvis-0.1*allmaxvis < 0)):
        ax[0].axis([0, uvmax, allminvis-0.1*allmaxvis, 1.1*allmaxvis])
        ax[1].axis([0, uvmax, allminvis-0.1*allmaxvis, 1.1*allmaxvis])
    else:
        ax[0].axis([0, uvmax, 0., 1.1*allmaxvis])
        ax[1].axis([0, uvmax, 0., 1.1*allmaxvis])

    ax[0].plot([0, uvmax], [0, 0], '--k')
    ax[1].plot([0, uvmax], [0, 0], '--k')
...
def estimate_flux_scale(reference, comparison, incl = 0, PA = 0, uvbins = None,
                        offx = 0, offy = 0, plot_label = None, show=False,
                        ymax=None):
    """
    ...
    plot_label: name of output figure saved to disk
    ymax: make an additional plot where the y axis is limited the ymax
    """
    ...
    print("#The ratio of the fluxes of %s to\n#%s is %.5f" % (comparison, reference, ratio_avg))
    print("#The scaling factor for gencal is %.3f for your comparison measurement" % (np.sqrt(ratio_avg)))
    print("#The error on the weighted mean ratio is %.3e, although it's likely that\n#the weights in the measurement sets are too off by some constant factor" % (1/np.sqrt(np.sum(w)),))
    print("")
    y_maxes = [None,]
    output_IDs = ['',]
    if ymax is not None:
        y_maxes.append(ymax)
        plot_label_split = os.path.basename(plot_label).split('.')
        assert len(plot_label_split) > 1
        extension = plot_label_split[-1]
        output_IDs.append(f'_ymax{ymax}.{extension}')
    for ymax_value,output_ID in zip(y_maxes,output_IDs):
        plt.figure()
        plt.title(f'{comparison}\n / {reference}')
        plt.errorbar(1e-3*rho_intersection, ratio, yerr = err, fmt = '.',
                     label = 'Binned ratios')
        plt.plot(1e-3*rho_intersection, np.ones_like(ratio)*ratio_avg,
                 label = f'weighted average ({ratio_avg:.3f})')
        plt.ylabel('Visibility amplitude ratios')
        plt.xlabel('UV distance (kilolambda)')
        plt.ylim(ymax=ymax_value)
        plt.legend()
        if plot_label is not None:
            fname = plot_label+output_ID
            plt.savefig(fname)
    if show:
        plt.show(block = False)
...
def get_station_numbers(msfile, antenna_name):
    ...
    for i in ant_numbers:
        matching_obs_1 = obsid[np.where(antenna1==i)]
        matching_obs_2 = obsid[np.where(antenna2==i)]
        matching_obs = np.unique(np.concatenate((matching_obs_1, matching_obs_2)))
...
def make_mask_from_region(image,region,output):
    makemask(mode='copy',inpimage=image,inpmask=region,output=output)

def generate_image_png(image,plot_sizes,color_scale_limits,save_folder=None,
                       image_units=None,image_center_offset=[0,0],
                       mask=None,noise_annulus=None):
    '''generate a png of the image, containing two plots: one showing the image
    with a color scale covering the whole range of values, and one with a color
    scale limited by the values of color_scale_limits
    image: path to the image
    plot_sizes: tuple containing the sizes of the two plots in [arcsec]
    color_scale_limits: min and max of the color scale for the scaled image, in the
    same units as the image (e.g. [Jy/beam])
    save_folder: folder where the png should be saved. If None, saved in the current folder
    image_units: if None, units printed in the fiugre title are taken from the image header
    image_center_offset: offset for the center of the output image [arcsec]
    noise_annulus: region string used to plot the noise annulus
    example:
        rms = 3e-5 #for this image, units are Jy/beam
        generate_image_pngs(image='CQ_Tau_SBLB_contap.image',
                            plot_sizes=[3,5],color_scale_limits=[-3*rms,10*rms])
    '''
    ia.open(image)
    summary = ia.summary()
    ia.close()
    center = summary['shape'][:2]//2
    assert np.all(summary['axisunits'][:2] == 'rad')
    pixelsize = np.abs(summary['incr'][:2])/constants.arcsec
    assert np.all(pixelsize==pixelsize[0])
    pixelsize = pixelsize[0]
    zooms = []
    image_center_offset = np.array(image_center_offset)
    for size in plot_sizes:
        blc = np.ceil(center - size/2/pixelsize - image_center_offset/pixelsize)
        trc = np.ceil(center + size/2/pixelsize - image_center_offset/pixelsize)
        #for some reason these need to be lists:
        zooms.append({'blc':list(blc),'trc':list(trc)})
    imview_images = [{'out':f'{image}.png','raster':{'file':image}},
                     {'out':f'{image}_noise_range.png',
                      'raster':{'file':image,
                                'range':color_scale_limits}}]
    contour_images = []
    to_delete = []
    if mask is not None:
        contour_images.append(mask)
    if noise_annulus is not None:
        noise_annulus_image = f'{image}.noise_annulus'
        make_mask_from_region(image=image,region=noise_annulus,
                              output=noise_annulus_image)
        contour_images.append(noise_annulus_image)
        to_delete.append(noise_annulus_image)
    if len(contour_images) == 0:
        pass
    elif len(contour_images) == 1:
        imview_images[1]['contour'] = {'file':contour_images[0]}
    elif len(contour_images) == 2:
        merged_contour_image = f'{image}.contours'
        immath(imagename=contour_images,mode='evalexpr',outfile=merged_contour_image,
               expr='iif(IM0+IM1==0,0,1)')
        to_delete.append(merged_contour_image)
        imview_images[1]['contour'] = {'file':merged_contour_image}
    else:
        raise RuntimeError
    for imview_image,zoom in zip(imview_images,zooms):
        imview_kwargs = {'raster':imview_image['raster'],'zoom':zoom,
                         'out':imview_image['out']}
        if 'contour' in imview_image:
            imview_kwargs['contour'] = imview_image['contour']
        imview(**imview_kwargs)
    for img in to_delete:
        shutil.rmtree(img)
    if image_units is None:
        image_units = imhead(image,mode='get',hdkey='bunit')
    titles = ['full data range',
              f'range {color_scale_limits[0]:.2g} to {color_scale_limits[1]:.2g} [{image_units}]']
    #to have less files, put the two images into a single file
    output_images = [matplotlib.image.imread(imview_image['out']) for imview_image
                     in imview_images]
    image_shapes = [t.shape for t in output_images]
    total_width = np.sum([shape[0] for shape in image_shapes])
    total_height = np.sum([shape[1] for shape in image_shapes])
    #unfortunately we cannot read the dpi directly; we would need to install the PIL
    #package inside CASA's python
    dpi = 100 #manually determined from imview-generated png
    #make figure a little larger than just total_width,total_height, to have
    #some additional space for title etc.
    plt.ioff() #turn off interactive mode so that no gui is opened
    fig,axes = plt.subplots(1,2,figsize=(1.5*total_width/dpi,1.5*total_height/dpi))
    for output_image,ax,title in zip(output_images,axes,titles):
        ax.imshow(output_image)
        ax.set_title(title,fontsize=16)
        for xy_ax in (ax.xaxis,ax.yaxis):
            xy_ax.set_visible('False')
            xy_ax.set_ticks([])
            xy_ax.set_ticklabels([])
    for imview_image in imview_images:
        os.remove(imview_image['out'])
    fname = f'{image}.png'
    if save_folder is not None:
        fname = os.path.join(save_folder,fname)
    fig.savefig(fname=fname,bbox_inches='tight',pad_inches=0.05,dpi=dpi)
    plt.close(fig)
    plt.ion()#turn on interactive mode again (not sure this is really needed)

def delete_tclean_output(imagename):
    """Remove the ouput of a tclean run. No guarantee that all outputs are caputred
    by the following endings!"""
    endings = ['pb','image.pbcor','mask','sumwt',
               'weight','gridwt_moswt','psf','alpha','alpha.error','beta',
               'workdirectory']
    #It is extra important to remove 'residual' and 'model'; if they exist, then
    #tclean will continue CLEANing, using them as starting point
    critical_endings = ['image','residual','model']
    for ending in endings+critical_endings:
        tclean_output = f'{imagename}.{ending}'
        if os.path.isdir(tclean_output):
            print(f'removing {tclean_output}')
            shutil.rmtree(tclean_output)
    for ending in critical_endings:
        assert not os.path.exists(f'{imagename}.{ending}')

def plot_amp_vs_time_comparison(nametemplate,visibilities,spw,uvrange,
                                output_folder=None):
    assert len(visibilities) == 2
    plotms_kwargs = {'xaxis':'time','ydatacolumn':'data','yaxis':'amp',
                     'uvrange':uvrange,'avgspw':True,'avgchannel':'10000',
                     'avgbaseline':True,'showgui':False}
    for corr in ('XX','YY'):
        plot_name = f'{nametemplate}_pol{corr}.png'
        plotms(visibilities[0],spw=spw,clearplots=True,correlation=corr,
               **plotms_kwargs)
        if output_folder is not None:
            plotfile = os.path.join(output_folder,plot_name)
        else:
            plotfile = plot_name
        plotms(visibilities[1],spw=spw,customsymbol=True,symbolcolor='red',plotindex=1,
               plotfile=plotfile,overwrite=True,clearplots=False,correlation=corr,
               **plotms_kwargs)
