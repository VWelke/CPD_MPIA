...
def process_single_mock(args):
    ...
    i, Fstr_i, mstr_i = args
    
    try:
        t0 = time.time()
        
        print(f"Starting mock {i+1}/{len(Fstr)}: {Fstr_i} μJy (PID: {os.getpid()})")
        ...
        ImportMS(r'D:/exoALMA_disk_data/data/' +target+'_time_ave_continuum.ms', rfile, suffix=resid_suffix, make_resid=False)
        ...
        print(f"✓ Mock {i+1} completed: {Fstr_i} μJy in {elapsed_time:.1f}s (PID: {os.getpid()})")
        
        return f"Mock {i+1}: {Fstr_i} μJy - SUCCESS ({elapsed_time:.1f}s)"
        
    except Exception as e:
        print(f"✗ Mock {i+1} FAILED: {str(e)} (PID: {os.getpid()})")
        return f"Mock {i+1}: {Fstr_i} μJy - FAILED: {str(e)}"

# Main parallel execution
print(f"\n=== Starting parallel imaging of {len(Fstr)} mocks ===")
print(f"Using {N_CORES_IMAGING} parallel processes with multiprocessing.Pool")
print(f"Target: {target}, Gap: {gap_ix}, Flux: {flux_bin_uJy} μJy")

# Record start time
pipeline_start = time.time()

# Create argument tuples for each mock
args_list = [(i, Fstr[i], mstr[i]) for i in range(len(Fstr))]

# Process all mocks in parallel using multiprocessing.Pool
with Pool(processes=N_CORES_IMAGING) as pool:
    results = pool.map(process_single_mock, args_list)
