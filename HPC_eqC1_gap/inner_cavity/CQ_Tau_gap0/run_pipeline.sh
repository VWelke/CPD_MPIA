...
WORKDIR="/nexus/posix0/MIA-astro-env/myben/vawelke/inj_rev/eqC1_gap/inner_cavity/CQ_Tau_gap0"
...
if [ -f "injections/CQ_Tau_gap0_mpars.0.txt" ]; then
    echo "=== [CQ_Tau_gap0] Inject already done, skipping ==="
else
    echo "=== [CQ_Tau_gap0] Inject start: $(date) ==="
    OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE MKL_NUM_THREADS=2 \
    OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
    python -u CQ_Tau_gap0_injectloop.py > inject.log 2>&1
...
if compgen -G "*.custom.mask" > /dev/null 2>&1; then
    ...
    echo "=== [CQ_Tau_gap0] Prepimaging start: $(date) ==="
...
echo "=== [CQ_Tau_gap0] Imageloop start: $(date) ==="
CASA_NUM_THREADS=2 OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE \
MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
casa --pipeline --configfile "$CASA_CONFIG" --nogui --nologger --nologfile \
  -c CQ_Tau_robust0_5_gap0_imageloop.py > imageloop.log 2>&1
echo "=== Imageloop finished: $(date) ===" >> imageloop.log

echo "=== [CQ_Tau_gap0] Recovery start: $(date) ==="
...
echo "=== [CQ_Tau_gap0] Pipeline complete: $(date) ==="
