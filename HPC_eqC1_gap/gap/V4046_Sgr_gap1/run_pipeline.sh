...
WORKDIR="/nexus/posix0/MIA-astro-env/myben/vawelke/inj_rev/eqC1_gap/gap/V4046_Sgr_gap1"
...
if [ -f "injections/V4046_Sgr_gap1_mpars.0.txt" ]; then
    echo "=== [V4046_Sgr_gap1] Inject already done, skipping ==="
else
    echo "=== [V4046_Sgr_gap1] Inject start: $(date) ==="
...
if compgen -G "*.custom.mask" > /dev/null 2>&1; then
    ...
    echo "=== [V4046_Sgr_gap1] Prepimaging start: $(date) ==="
...
echo "=== [V4046_Sgr_gap1] Imageloop start: $(date) ==="
CASA_NUM_THREADS=2 OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE \
MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
...
echo "=== [V4046_Sgr_gap1] Recovery start: $(date) ==="
...
echo "=== [V4046_Sgr_gap1] Pipeline complete: $(date) ==="
