...
WORKDIR="/nexus/posix0/MIA-astro-env/myben/vawelke/inj_rev/eqC1_gap/gap/AA_Tau_gap0"
...
if [ -f "injections/AA_Tau_gap0_mpars.0.txt" ]; then
    echo "=== [AA_Tau_gap0] Inject already done, skipping ==="
else
    echo "=== [AA_Tau_gap0] Inject start: $(date) ==="
    OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE MKL_NUM_THREADS=2 \
    OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
    python -u AA_Tau_gap0_injectloop.py > inject.log 2>&1
...
if compgen -G "*.custom.mask" > /dev/null 2>&1; then
    ...
    echo "=== [AA_Tau_gap0] Prepimaging start: $(date) ==="
...
echo "=== [AA_Tau_gap0] Imageloop start: $(date) ==="
CASA_NUM_THREADS=2 OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE \
MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
casa --pipeline --configfile "$CASA_CONFIG" --nogui --nologger --nologfile \
  -c AA_Tau_robust0_5_gap0_imageloop.py > imageloop.log 2>&1
echo "=== Imageloop finished: $(date) ===" >> imageloop.log

echo "=== [AA_Tau_gap0] Recovery start: $(date) ==="
...
echo "=== [AA_Tau_gap0] Pipeline complete: $(date) ==="
