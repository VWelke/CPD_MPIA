...
WORKDIR="/nexus/posix0/MIA-astro-env/myben/vawelke/inj_rev/eqC1_gap/gap/MWC_758_gap1"
...
if [ -f "injections/MWC_758_gap1_mpars.0.txt" ]; then
    echo "=== [MWC_758_gap1] Inject already done, skipping ==="
else
    echo "=== [MWC_758_gap1] Inject start: $(date) ==="
    OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE MKL_NUM_THREADS=2 \
    OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
    python -u MWC_758_gap1_injectloop.py > inject.log 2>&1
...
if compgen -G "*.custom.mask" > /dev/null 2>&1; then
    ...
    echo "=== [MWC_758_gap1] Prepimaging start: $(date) ==="
...
echo "=== [MWC_758_gap1] Imageloop start: $(date) ==="
CASA_NUM_THREADS=2 OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE \
MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
casa --pipeline --configfile "$CASA_CONFIG" --nogui --nologger --nologfile \
  -c MWC_758_robust0_5_gap1_imageloop.py > imageloop.log 2>&1
echo "=== Imageloop finished: $(date) ===" >> imageloop.log

echo "=== [MWC_758_gap1] Recovery start: $(date) ==="
...
echo "=== [MWC_758_gap1] Pipeline complete: $(date) ==="
