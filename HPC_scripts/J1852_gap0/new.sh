...
WORKDIR="/nexus/posix0/MIA-astro-env/myben/vawelke/inj_rev/J1852_gap0"
...
cd "$WORKDIR"
mkdir -p resid_vis mprofiles recoveries
source "$VENV"

export PATH="${CASA_BIN}:$PATH"

echo "=== Inject start: $(date) ==="
...
echo "=== Imageloop start: $(date) ==="
CASA_NUM_THREADS=2 OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
casa --pipeline --configfile "$CASA_CONFIG" --nogui --nologger --nologfile \
  -c J1852_robust2_0_gap0_imageloop.py > imageloop.log 2>&1
