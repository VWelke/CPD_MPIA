...
WORKDIR="/nexus/posix0/MIA-astro-env/myben/vawelke/inj_rev/r0_5/J1852_gap0_test_rout"
...
if [ -f "injections/J1852_gap0_mpars.0.txt" ]; then
    echo "=== [J1852 gap0] Inject already done, skipping ==="
else
    echo "=== [J1852 gap0] Inject start: $(date) ==="
...
if compgen -G "*.custom.mask" > /dev/null 2>&1; then
    ...
    echo "=== [J1852 gap0] Prepimaging start: $(date) ==="
...
echo "=== [J1852 gap0] Imageloop start: $(date) ==="
CASA_NUM_THREADS=2 OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
...
echo "=== [J1852 gap0] Recovery start: $(date) ==="
...
echo "=== [J1852 gap0] Pipeline complete: $(date) ==="
