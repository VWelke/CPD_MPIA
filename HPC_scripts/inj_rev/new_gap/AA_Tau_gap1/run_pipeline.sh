...
WORKDIR="/nexus/posix0/MIA-astro-env/myben/vawelke/inj_rev/new_gap/AA_Tau_gap1"
...
if [ -f "injections/AA_Tau_gap1_mpars.0.txt" ]; then
    echo "=== [AA_Tau gap2] Inject already done, skipping ==="
else
    echo "=== [AA_Tau gap2] Inject start: $(date) ==="
...
if compgen -G "*.custom.mask" > /dev/null 2>&1; then
    ...
    echo "=== [AA_Tau gap2] Prepimaging start: $(date) ==="
...
echo "=== [AA_Tau gap2] Imageloop start: $(date) ==="
CASA_NUM_THREADS=2 OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
...
echo "=== [AA_Tau gap2] Recovery start: $(date) ==="
...
echo "=== [AA_Tau gap2] Pipeline complete: $(date) ==="
