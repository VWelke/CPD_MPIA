...
WORKDIR="/nexus/posix0/MIA-astro-env/myben/vawelke/inj_rev/r0_5_rout/HD_34282_gap2"
...
if [ -f "injections/HD_34282_gap2_mpars.0.txt" ]; then
    echo "=== [HD_34282 gap2] Inject already done, skipping ==="
else
    echo "=== [HD_34282 gap2] Inject start: $(date) ==="
    OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
    python -u HD_34282_gap2_injectloop.py > inject.log 2>&1
...
if compgen -G "*.custom.mask" > /dev/null 2>&1; then
    ...
    echo "=== [HD_34282 gap2] Prepimaging start: $(date) ==="
...
echo "=== [HD_34282 gap2] Imageloop start: $(date) ==="
CASA_NUM_THREADS=2 OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
casa --pipeline --configfile "$CASA_CONFIG" --nogui --nologger --nologfile \
  -c HD_34282_robust0_5_gap2_imageloop.py > imageloop.log 2>&1
echo "=== Imageloop finished: $(date) ===" >> imageloop.log

echo "=== [HD_34282 gap2] Recovery start: $(date) ==="
...
echo "=== [HD_34282 gap2] Pipeline complete: $(date) ==="
