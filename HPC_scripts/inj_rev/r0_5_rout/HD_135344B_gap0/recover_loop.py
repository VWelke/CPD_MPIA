...
for i in range(len(Fstr)):
    ...
    hdu = fits.open(f'/nexus/posix0/MIA-astro-env/myben/vawelke/inj_rev/r0_5_rout/{target}_gap{gap}/resid_images/' + im_file + '.resid.fits')
    ...
    xs, ys = np.meshgrid(RAo - disk.disk[target]['dx'], DECo - disk.disk[target]['dy'])
