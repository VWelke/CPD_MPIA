...
target, gap_ix, subsuf = 'HD_135344B', '0', '0'
...
imdir = f'/nexus/posix0/MIA-astro-env/myben/vawelke/inj_rev/r0_5_rout/{target}_gap{gap_ix}/resid_images/'
...
vdat = np.load('/nexus/posix0/MIA-astro-env/myben/vawelke/exoALMA_disk_data/measurement_set_spavg/npz/'+target+'_time_ave_continuum_spavg_lambda.vis.npz')
...
for i in range(len(Fcpd)):
    in_f = (Fi == Fcpd[i])
    pf_astA[i] = np.max((np.pi*np.average(dlim_A[in_f])**2 / beam_area, 1)) / (4 * np.pi * rgap * wgap / beam_area)
    pf_astB[i] = np.max((np.pi*np.average(dlim_B[in_f])**2 / beam_area, 1)) / (4 * np.pi * rgap * wgap / beam_area)
...
np.savetxt('recoveries/'+target+'_gap'+str(gap_ix)+'_rprofs.'+subsuf+'.txt',
           list(zip(Fcpd, frec_A, efrec_A, false_A, frec_B, efrec_B, false_B)), fmt='%.4f')

fig, ax = plt.subplots()
ax.errorbar(Fcpd, frec_A, yerr=efrec_A, marker='o', color='b', ls='none', capsize=0.0, elinewidth=1.5)
ax.errorbar(Fcpd, frec_B, yerr=efrec_B, marker='o', color='r', ls='none', capsize=0.0, elinewidth=1.5)
ax.set_xlim([0, 260]); ax.set_ylim([0, 1.0])
ax.set_xlabel('injected flux (microJy)'); ax.set_ylabel('recovery fraction')
os.makedirs('assess_figs', exist_ok=True)
fig.savefig('assess_figs/'+target+'_recovery_profiles.png'); fig.clf()
...
fig, ax = plt.subplots()
ax.errorbar(Fcpd, mu_A, yerr=sig_A, color='b', ls='none', marker='o', capsize=0.0)
ax.errorbar(Fcpd, mu_B, yerr=sig_B, color='r', ls='none', marker='o', capsize=0.0)
ax.plot([0, 260], [0, 0], ':k')
ax.set_xlim([0, 260]); ax.set_ylim([-100, 100])
fig.savefig('assess_figs/'+target+'_Fbias.png'); fig.clf()
...
for i in range(1,6):
    ...
    xd = fails_Ao[1] * np.cos(np.radians(fails_Ao[2])); yd = fails_Ao[1] * np.sin(np.radians(fails_Ao[2]))
    xs =  xd * np.cos(PAr) * np.cos(inclr) + yd * np.sin(PAr)
    ys = -xd * np.sin(PAr) * np.cos(inclr) + yd * np.cos(PAr)
    DR = np.sqrt(xs**2 + ys**2); DT = 90 - np.degrees(np.arctan2(ys, xs))
    print(('crit A: F = %i, r = %.3f, az = %i, rho=%.3f, PA = %i  (N = %i) ' % (fails_Ao[0], fails_Ao[1], fails_Ao[2], DR, DT, nctsA)))
    nfailsA += nctsA
print(('fraction of crit A fails = %.2f' % (1.*nfailsA / len(FnA))))
print(' ')
for i in range(1, 6):
    fails_Bo, nctsB = valsB[ctsB == np.sort(ctsB)[-i]][0], np.sort(ctsB)[-i]
    xd = fails_Bo[1] * np.cos(np.radians(fails_Bo[2])); yd = fails_Bo[1] * np.sin(np.radians(fails_Bo[2]))
    xs =  xd * np.cos(PAr) * np.cos(inclr) + yd * np.sin(PAr)
    ys = -xd * np.sin(PAr) * np.cos(inclr) + yd * np.cos(PAr)
    DR = np.sqrt(xs**2 + ys**2); DT = 90 - np.degrees(np.arctan2(ys, xs))
    print(('crit B: F = %i, r = %.3f, az = %i, rho=%.3f, PA =%i  (N = %i) ' % (fails_Bo[0], fails_Bo[1], fails_Bo[2], DR, DT, nctsB)))
    nfailsB += nctsB
