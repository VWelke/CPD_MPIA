...
WORKDIR="/nexus/posix0/MIA-astro-env/myben/vawelke/inj_rev/r0_5_rout/MWC_758_gap1"
...
if [ -f "injections/MWC_758_gap1_mpars.0.txt" ]; then
    echo "=== [MWC_758 gap1] Inject already done, skipping ==="
else
    echo "=== [MWC_758 gap1] Inject start: $(date) ==="
...
if compgen -G "*.custom.mask" > /dev/null 2>&1; then
    ...
    echo "=== [MWC_758 gap1] Prepimaging start: $(date) ==="
...
echo "=== [MWC_758 gap1] Imageloop start: $(date) ==="
CASA_NUM_THREADS=2 OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
...
echo "=== [MWC_758 gap1] Recovery start: $(date) ==="
...
echo "=== [MWC_758 gap1] Pipeline complete: $(date) ==="
