...
WORKDIR="/nexus/posix0/MIA-astro-env/myben/vawelke/inj_rev/r0_5_rout/LkCa_15_gap1"
...
if [ -f "injections/LkCa_15_gap1_mpars.0.txt" ]; then
    echo "=== [LkCa_15 gap1] Inject already done, skipping ==="
else
    echo "=== [LkCa_15 gap1] Inject start: $(date) ==="
    OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
    python -u LkCa_15_gap1_injectloop.py > inject.log 2>&1
    echo "=== Inject finished: $(date) ===" >> inject.log
fi

echo "=== [LkCa_15 gap1] Prepimaging start: $(date) ==="
...
echo "=== [LkCa_15 gap1] Imageloop start: $(date) ==="
CASA_NUM_THREADS=2 OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
casa --pipeline --configfile "$CASA_CONFIG" --nogui --nologger --nologfile \
  -c LkCa_15_robust0_5_gap1_imageloop.py > imageloop.log 2>&1
echo "=== Imageloop finished: $(date) ===" >> imageloop.log

echo "=== [LkCa_15 gap1] Recovery start: $(date) ==="
...
echo "=== [LkCa_15 gap1] Pipeline complete: $(date) ==="
