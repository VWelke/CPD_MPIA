...
WORKDIR="/nexus/posix0/MIA-astro-env/myben/vawelke/inj_rev/r0_5_rout/DM_Tau_gap2"
...
if [ -f "injections/DM_Tau_gap2_mpars.0.txt" ]; then
    echo "=== [DM_Tau gap2] Inject already done, skipping ==="
else
    echo "=== [DM_Tau gap2] Inject start: $(date) ==="
    OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
    python -u DM_Tau_gap2_injectloop.py > inject.log 2>&1
    echo "=== Inject finished: $(date) ===" >> inject.log
fi

echo "=== [DM_Tau gap2] Prepimaging start: $(date) ==="
...
echo "=== [DM_Tau gap2] Imageloop start: $(date) ==="
CASA_NUM_THREADS=2 OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
casa --pipeline --configfile "$CASA_CONFIG" --nogui --nologger --nologfile \
  -c DM_Tau_robust0_5_gap2_imageloop.py > imageloop.log 2>&1
echo "=== Imageloop finished: $(date) ===" >> imageloop.log

echo "=== [DM_Tau gap2] Recovery start: $(date) ==="
...
echo "=== [DM_Tau gap2] Pipeline complete: $(date) ==="
