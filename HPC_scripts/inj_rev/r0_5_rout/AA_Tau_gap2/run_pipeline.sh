...
WORKDIR="/nexus/posix0/MIA-astro-env/myben/vawelke/inj_rev/r0_5_rout/AA_Tau_gap2"
...
if [ -f "injections/AA_Tau_gap2_mpars.0.txt" ]; then
    ...
    python -u AA_Tau_gap2_injectloop.py > inject.log 2>&1
...
casa --pipeline --configfile "$CASA_CONFIG" --nogui --nologger --nologfile \
  -c AA_Tau_robust0_5_gap2_imageloop.py > imageloop.log 2>&1
