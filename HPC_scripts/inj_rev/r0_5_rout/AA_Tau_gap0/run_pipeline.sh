...
WORKDIR="/nexus/posix0/MIA-astro-env/myben/vawelke/inj_rev/r0_5_rout/AA_Tau_gap0"
...
if [ -f "injections/AA_Tau_gap0_mpars.0.txt" ]; then
    echo "=== [AA_Tau gap0] Inject already done, skipping ==="
else
    echo "=== [AA_Tau gap0] Inject start: $(date) ==="
...
if compgen -G "*.custom.mask" > /dev/null 2>&1; then
    ...
    echo "=== [AA_Tau gap0] Prepimaging start: $(date) ==="
...
echo "=== [AA_Tau gap0] Imageloop start: $(date) ==="
CASA_NUM_THREADS=2 OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
...
echo "=== [AA_Tau gap0] Recovery start: $(date) ==="
...
echo "=== [AA_Tau gap0] Pipeline complete: $(date) ==="
