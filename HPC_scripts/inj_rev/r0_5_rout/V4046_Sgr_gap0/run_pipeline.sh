...
WORKDIR="/nexus/posix0/MIA-astro-env/myben/vawelke/inj_rev/r0_5_rout/V4046_Sgr_gap0"
...
if [ -f "injections/V4046_Sgr_gap0_mpars.0.txt" ]; then
    echo "=== [V4046_Sgr gap0] Inject already done, skipping ==="
else
    echo "=== [V4046_Sgr gap0] Inject start: $(date) ==="
...
echo "=== [V4046_Sgr gap0] Prepimaging start: $(date) ==="
...
echo "=== [V4046_Sgr gap0] Imageloop start: $(date) ==="
CASA_NUM_THREADS=2 OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
...
echo "=== [V4046_Sgr gap0] Recovery start: $(date) ==="
...
echo "=== [V4046_Sgr gap0] Pipeline complete: $(date) ==="
