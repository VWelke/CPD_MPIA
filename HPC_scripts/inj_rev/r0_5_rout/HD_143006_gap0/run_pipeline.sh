...
WORKDIR="/nexus/posix0/MIA-astro-env/myben/vawelke/inj_rev/r0_5_rout/HD_143006_gap0"
...
if [ -f "injections/HD_143006_gap0_mpars.0.txt" ]; then
    echo "=== [HD_143006 gap0] Inject already done, skipping ==="
else
    echo "=== [HD_143006 gap0] Inject start: $(date) ==="
    OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
    python -u HD_143006_gap0_injectloop.py > inject.log 2>&1
    echo "=== Inject finished: $(date) ===" >> inject.log
fi

echo "=== [HD_143006 gap0] Prepimaging start: $(date) ==="
...
echo "=== [HD_143006 gap0] Imageloop start: $(date) ==="
CASA_NUM_THREADS=2 OMP_NUM_THREADS=2 OMP_DYNAMIC=FALSE MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 NUMEXPR_NUM_THREADS=2 \
casa --pipeline --configfile "$CASA_CONFIG" --nogui --nologger --nologfile \
  -c HD_143006_robust0_5_gap0_imageloop.py > imageloop.log 2>&1
echo "=== Imageloop finished: $(date) ===" >> imageloop.log

echo "=== [HD_143006 gap0] Recovery start: $(date) ==="
...
echo "=== [HD_143006 gap0] Pipeline complete: $(date) ==="
