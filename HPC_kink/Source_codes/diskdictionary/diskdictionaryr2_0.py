# Disk dictionaries for kink injection-recovery, robust=2.0
# rgap/wgap hold kink positions (arcsec) — key names kept for pipeline compatibility
# Kink positions from kink_sources in Median_SNR/Dictionaries.py (AU / distance_pc)
# NOTE: J1615 kink at 1.987 arcsec is at/beyond rout for this robust value

import numpy as np

disk = {
        'AA_Tau': {'PA': 93.77079777,
            'RMS': np.float64(23.725058781565167),
            'ccycleniter': 300,
            'cgain': 0.3,
            'cmask': 'ellipse[[512pix,512pix], [2arcsec, '
                     '1.0439459414332437arcsec], 93.77079777deg]',
            'crobust': 2.0,
            'cscale': [0, 8, 15, 30, 80],
            'ctaper': [],
            'cthresh': '0.119mJy',
            'dgap': [0.0],
            'distance': 135,
            'dx': -0.00545897,
            'dy': 0.00482739,
            'gscales': [0, 5],
            'gthresh': '0.047mJy',
            'hyp-Ncoll': 300,
            'hyp-alpha': 1.3,
            'hyp-wsmth': 0.1,
            'incl': 58.53531224,
            'label': 'AA Tau',
            'lstar': 1.1,
            'mstar': 0.79,
            'name': 'AA_Tau',
            'rgap': [0.593],
            'rout': np.float64(1.2774412985891628),
            'wgap': [0.1]},
        'CQ_Tau': {'PA': 53.87180444,
                'RMS': np.float64(25.65815157140605),
                'ccycleniter': 300,
                'cgain': 0.3,
                'cmask': 'ellipse[[512pix,512pix], [2arcsec, '
                        '1.633432101799876arcsec], 53.87180444deg]',
                'crobust': 2.0,
                'cscale': [0, 8, 15, 30, 80],
                'ctaper': [],
                'cthresh': '0.128mJy',
                'dgap': [],
                'distance': 149,
                'dx': -0.00871044,
                'dy': 0.0009941,
                'gthresh': [],
                'hyp-Ncoll': 300,
                'hyp-alpha': 1.3,
                'hyp-wsmth': 0.1,
                'incl': 35.2426038,
                'label': 'CQ Tau',
                'lstar': 10,
                'mstar': 1.4,
                'name': 'CQ_Tau',
                'rgap': [],
                'rout': np.float64(0.8167716581373495),
                'wgap': []},
        'DM_Tau': {'PA': 155.59975598,
                'RMS': np.float64(21.415021365100984),
                'ccycleniter': 300,
                'cgain': 0.3,
                'cmask': 'ellipse[[512pix,512pix], [2arcsec, '
                        '1.6187017651640194arcsec], 155.59975598deg]',
                'crobust': 2.0,
                'cscale': [0, 8, 15, 30, 80],
                'ctaper': [],
                'cthresh': '0.107mJy',
                'dgap': [],
                'distance': 144,
                'dx': -0.00551499,
                'dy': -0.00658999,
                'gthresh': [],
                'hyp-Ncoll': 300,
                'hyp-alpha': 1.3,
                'hyp-wsmth': 0.1,
                'incl': 35.96744071,
                'label': 'DM Tau',
                'lstar': 0.24,
                'mstar': 0.45,
                'name': 'DM_Tau',
                'rgap': [],
                'rout': np.float64(1.712675878778223),
                'wgap': []},
        'HD_135344B': {'PA': 28.92070668,
                        'RMS': np.float64(19.780339243880007),
                        'ccycleniter': 300,
                        'cgain': 0.3,
                        'cmask': 'ellipse[[512pix,512pix], [2arcsec, '
                                '1.8704829781176846arcsec], 28.92070668deg]',
                        'crobust': 2.0,
                        'cscale': [0, 8, 15, 30, 80],
                        'ctaper': [],
                        'cthresh': '0.099mJy',
                        'dgap': [],
                        'distance': 135,
                        'dx': 0.0007974,
                        'dy': -0.00320815,
                        'gthresh': [],
                        'hyp-Ncoll': 300,
                        'hyp-alpha': 1.3,
                        'hyp-wsmth': 0.1,
                        'incl': 20.73280574,
                        'label': 'HD 135344B',
                        'lstar': 6.7,
                        'mstar': 1.61,
                        'name': 'HD_135344B',
                        'rgap': [],
                        'rout': np.float64(1.1283771805464915),
                        'wgap': []},
        'HD_143006': {'PA': 7.53236754,
                'RMS': np.float64(20.46369445452001),
                'ccycleniter': 300,
                'cgain': 0.3,
                'cmask': 'ellipse[[512pix,512pix], [2arcsec, '
                                '1.8944886188220467arcsec], 7.53236754deg]',
                'crobust': 2.0,
                'cscale': [0, 8, 15, 30, 80],
                'ctaper': [],
                'cthresh': '0.102mJy',
                'dgap': [0.0],
                'distance': 167,
                'dx': 0.00826703,
                'dy': 0.02642291,
                'gscales': [0, 5],
                'gthresh': '0.041mJy',
                'hyp-Ncoll': 300,
                'hyp-alpha': 1.3,
                'hyp-wsmth': 0.1,
                'incl': 18.69391735,
                'label': 'HD 143006',
                'lstar': 3.8,
                'mstar': 1.56,
                'name': 'HD_143006',
                'rgap': [0.192],
                'rout': np.float64(0.7983610965311625),
                'wgap': [0.05]},
        'HD_34282': {'PA': 117.79519232,
                'RMS': np.float64(21.297418243193533),
                'ccycleniter': 300,
                'cgain': 0.3,
                'cmask': 'ellipse[[512pix,512pix], [2arcsec, '
                        '1.005527473149872arcsec], 117.79519232deg]',
                'crobust': 2.0,
                'cscale': [0, 8, 15, 30, 80],
                'ctaper': [],
                'cthresh': '0.106mJy',
                'dgap': [],
                'distance': 309,
                'dx': 0.01282042,
                'dy': 0.00837812,
                'gthresh': [],
                'hyp-Ncoll': 300,
                'hyp-alpha': 1.3,
                'hyp-wsmth': 0.1,
                'incl': 59.81698359,
                'label': 'HD 34282',
                'lstar': 108,
                'mstar': 1.61,
                'name': 'HD_34282',
                'rgap': [],
                'rout': np.float64(1.2095795134081664),
                'wgap': []},
        'J1604': {'PA': 123.24221853,
                'RMS': np.float64(22.730672753823455),
                'ccycleniter': 300,
                'cgain': 0.3,
                'cmask': 'ellipse[[512pix,512pix], [2arcsec, '
                        '1.9768678096037415arcsec], 123.24221853deg]',
                'crobust': 2.0,
                'cscale': [0, 8, 15, 30, 80],
                'ctaper': [],
                'cthresh': '0.114mJy',
                'dgap': [],
                'distance': 145,
                'dx': -0.07482211,
                'dy': -0.01666589,
                'gthresh': [],
                'hyp-Ncoll': 300,
                'hyp-alpha': 1.3,
                'hyp-wsmth': 0.1,
                'incl': 8.7226911,
                'label': 'J1604',
                'lstar': 0.76,
                'mstar': 1.29,
                'name': 'J1604',
                'rgap': [],
                'rout': np.float64(0.035053588449954),
                'wgap': []},
        'J1615': {'PA': 146.14327945,
                'RMS': np.float64(17.710814063320868),
                'ccycleniter': 300,
                'cgain': 0.3,
                'cmask': 'ellipse[[512pix,512pix], [2arcsec, '
                        '1.3614990911539273arcsec], 146.14327945deg]',
                'crobust': 2.0,
                'cscale': [0, 8, 15, 30, 80],
                'ctaper': [],
                'cthresh': '0.089mJy',
                'dgap': [0.0],
                'distance': 156,
                'dx': -0.04431726,
                'dy': -0.00588429,
                'gscales': [0, 5],
                'gthresh': '0.035mJy',
                'hyp-Ncoll': 300,
                'hyp-alpha': 1.3,
                'hyp-wsmth': 0.1,
                'incl': 47.09775702,
                'label': 'J1615',
                'lstar': 1.07,
                'mstar': 1.14,
                'name': 'J1615',
                'rgap': [1.987],
                'rout': np.float64(1.970932420343169),
                'wgap': [0.15]},
        'J1842': {'PA': 26.35016474,
                'RMS': np.float64(19.370352674741298),
                'ccycleniter': 300,
                'cgain': 0.3,
                'cmask': 'ellipse[[512pix,512pix], [2arcsec, '
                        '1.549481012108344arcsec], 26.35016474deg]',
                'crobust': 2.0,
                'cscale': [0, 8, 15, 30, 80],
                'ctaper': [],
                'cthresh': '0.097mJy',
                'dgap': [0.0],
                'distance': 151,
                'dx': -0.00316491,
                'dy': -0.03068915,
                'gscales': [0, 5],
                'gthresh': '0.039mJy',
                'hyp-Ncoll': 300,
                'hyp-alpha': 1.3,
                'hyp-wsmth': 0.1,
                'incl': 39.21848812,
                'label': 'J1842',
                'lstar': 0.8,
                'mstar': 1.07,
                'name': 'J1842',
                'rgap': [0.695],
                'rout': np.float64(1.002886492759056),
                'wgap': [0.1]},
        'J1852': {'PA': 117.61494041,
                'RMS': np.float64(16.89891632850049),
                'ccycleniter': 300,
                'cgain': 0.3,
                'cmask': 'ellipse[[512pix,512pix], [2arcsec, '
                        '1.6868119485868183arcsec], 117.61494041deg]',
                'crobust': 2.0,
                'cscale': [0, 8, 15, 30, 80],
                'ctaper': [],
                'cthresh': '0.084mJy',
                'dgap': [],
                'distance': 147,
                'dx': -0.02340882,
                'dy': 0.00190753,
                'gthresh': [],
                'hyp-Ncoll': 300,
                'hyp-alpha': 1.3,
                'hyp-wsmth': 0.1,
                'incl': 32.4984507,
                'label': 'J1852',
                'lstar': 0.6,
                'mstar': 1.03,
                'name': 'J1852',
                'rgap': [],
                'rout': np.float64(0.8812480866908894),
                'wgap': []},
        'LkCa_15': {'PA': 61.57232522,
                'RMS': np.float64(19.101446014246903),
                'ccycleniter': 300,
                'cgain': 0.3,
                'cmask': 'ellipse[[512pix,512pix], [2arcsec, '
                        '1.2695975167543332arcsec], 61.57232522deg]',
                'crobust': 2.0,
                'cscale': [0, 8, 15, 30, 80],
                'ctaper': [],
                'cthresh': '0.096mJy',
                'dgap': [0.0],
                'distance': 156,
                'dx': -0.01684397,
                'dy': 0.02082818,
                'gscales': [0, 5],
                'gthresh': '0.038mJy',
                'hyp-Ncoll': 300,
                'hyp-alpha': 1.3,
                'hyp-wsmth': 0.1,
                'incl': 50.59493965,
                'label': 'LkCa 15',
                'lstar': 1,
                'mstar': 1.14,
                'name': 'LkCa_15',
                'rgap': [1.538],
                'rout': np.float64(1.480163607746208),
                'wgap': [0.15]},
        'MWC_758': {'PA': 76.17365568,
                'RMS': np.float64(25.907760573318228),
                'ccycleniter': 300,
                'cgain': 0.3,
                'cmask': 'ellipse[[512pix,512pix], [2arcsec, '
                        '1.9839420811003194arcsec], 76.17365568deg]',
                'crobust': 2.0,
                'cscale': [0, 8, 15, 30, 80],
                'ctaper': [],
                'cthresh': '0.130mJy',
                'dgap': [],
                'distance': 156,
                'dx': 0.02548176,
                'dy': 0.01841853,
                'gthresh': [],
                'hyp-Ncoll': 300,
                'hyp-alpha': 1.3,
                'hyp-wsmth': 0.1,
                'incl': 7.26537891,
                'label': 'MWC 758',
                'lstar': 10.4,
                'mstar': 1.4,
                'name': 'MWC_758',
                'rgap': [],
                'rout': np.float64(0.89012095704675),
                'wgap': []},
        'PDS_66': {'PA': 8.91316735,
                'RMS': np.float64(25.129824280156754),
                'ccycleniter': 300,
                'cgain': 0.3,
                'cmask': 'ellipse[[512pix,512pix], [2arcsec, '
                        '1.6957384414086927arcsec], 8.91316735deg]',
                'crobust': 2.0,
                'cscale': [0, 8, 15, 30, 80],
                'ctaper': [],
                'cthresh': '0.126mJy',
                'dgap': [],
                'distance': 98,
                'dx': -0.00358921,
                'dy': 0.00667634,
                'gthresh': [],
                'hyp-Ncoll': 300,
                'hyp-alpha': 1.3,
                'hyp-wsmth': 0.1,
                'incl': 32.0193351,
                'label': 'PDS 66',
                'lstar': 1.2,
                'mstar': 1.28,
                'name': 'PDS_66',
                'rgap': [],
                'rout': np.float64(0.9483695290983059),
                'wgap': []},
        'SY_Cha': {'PA': 165.76511321,
                'RMS': np.float64(26.288028493581805),
                'ccycleniter': 300,
                'cgain': 0.3,
                'cmask': 'ellipse[[512pix,512pix], [2arcsec, '
                        '1.2410252268065851arcsec], 165.76511321deg]',
                'crobust': 2.0,
                'cscale': [0, 8, 15, 30, 80],
                'ctaper': [],
                'cthresh': '0.131mJy',
                'dgap': [0.0],
                'distance': 182,
                'dx': -0.01265537,
                'dy': 0.02815886,
                'gscales': [0, 5],
                'gthresh': '0.053mJy',
                'hyp-Ncoll': 300,
                'hyp-alpha': 1.3,
                'hyp-wsmth': 0.1,
                'incl': 51.64642211,
                'label': 'SY Cha',
                'lstar': 0.55,
                'mstar': 0.77,
                'name': 'SY_Cha',
                'rgap': [0.769],
                'rout': np.float64(1.5062807556242503),
                'wgap': [0.1]},
        'V4046_Sgr': {'PA': 76.02083693,
                'RMS': np.float64(20.42987762251869),
                'ccycleniter': 300,
                'cgain': 0.3,
                'cmask': 'ellipse[[512pix,512pix], [2arcsec, '
                                '1.6704810703258652arcsec], 76.02083693deg]',
                'crobust': 2.0,
                'cscale': [0, 8, 15, 30, 80],
                'ctaper': [],
                'cthresh': '0.102mJy',
                'dgap': [],
                'distance': 72,
                'dx': -0.05093653,
                'dy': -0.04518314,
                'gthresh': [],
                'hyp-Ncoll': 300,
                'hyp-alpha': 1.3,
                'hyp-wsmth': 0.1,
                'incl': 33.35910733,
                'label': 'V4046 Sgr',
                'lstar': 0.5,
                'mstar': 1.73,
                'name': 'V4046_Sgr',
                'rgap': [],
                'rout': np.float64(1.6814195029437211),
                'wgap': []}
        }
