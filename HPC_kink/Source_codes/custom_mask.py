...
def custom_mask(name, gix, filecopy, buffer_factor=1.0, feature='gap'):
    ...
    # identify feature parameters (gap or kink)
    rfeat = disk.disk[name]['r' + feature][gix]
    wfeat = disk.disk[name]['w' + feature][gix]

    # feature boundaries in polar disk-plane coordinates
    bndi = (rd >= (rfeat - buffer_factor * wfeat))
    bndo = (rd <= (rfeat + buffer_factor * wfeat))
