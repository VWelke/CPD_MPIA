...
for i in range(len(Fstr)):
    ...
    resid_suffix = 'kink'+kink_ix+'.F'+Fstr[i]+'uJy_'+mstr[i]+'.resid'
    ...
    os.system('cp -r '+target+'_kink'+kink_ix+'.'+subsuf+'.psf ' + \
              im_outfile+'.psf')
    os.system('cp -r '+target+'_kink'+kink_ix+'.'+subsuf+'.sumwt ' + \
              im_outfile+'.sumwt')
    ...
    tclean(vis='/nexus/posix0/MIA-astro-env/myben/vawelke/exoALMA_disk_data/measurement_set_spavg/'+target+'_time_ave_continuum_spavg.'+resid_suffix+'.ms',
       ...
       mask=target+'_kink'+kink_ix+'.'+subsuf+'.custom.mask',
