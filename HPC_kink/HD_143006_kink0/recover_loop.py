...
inj_file = 'injections/'+target+'_kink'+str(kink)+'_mpars.'+ix+'.txt'
Fstr, mstr, rstr, azstr = np.loadtxt(inj_file, dtype=str).T
Fcpd, mdl, rcpd, azcpd = np.loadtxt(inj_file).T

# bookkeeping
recov_file = 'recoveries/'+target+'_kink'+str(kink)+'_recoveries.'+ix+'.txt'
...
for i in range(len(Fstr)):
    ...
    # Load kink properties
    rkink = disk.disk[target]['rkink'][kink]
    wkink = disk.disk[target]['wkink'][kink]

    # Apply a boolean mask to isolate the search annulus
    mask = np.zeros_like(img, dtype='bool')
    bndi, bndo = (rd >= (rkink - wkink)), (rd <= (rkink + wkink))
    ...
    # Quantify the pixel distribution in the kink region
