...
# specify target disk and kink
target = 'HD_143006'  # CSD name
kink_ix = 0           # which kink CPD is in (based on dict list)
...
os.system('rm -rf '+target+'_kink'+str(kink_ix)+'_mpars.'+subsuf+'.txt')
...
for i in range(len(F_cpd)):
    ...
    rkink_cen = disk.disk[target]['rkink'][kink_ix]
    kink_span = 0.5 * disk.disk[target]['wkink'][kink_ix]
    r_cpd = np.random.uniform(rkink_cen - kink_span, rkink_cen + kink_span,
                              n_mocks_per_F)
    ...
    for j in range(n_mocks_per_F):
        ...
        save_fit(u, v, vis_cpd, wgt, sol,
                 prefix=target+'_kink'+str(kink_ix)+file_suffix,
                 save_vis_fit=False, save_solution=False)

        # clean up file outputs
        os.system('mv '+target+'_kink'+str(kink_ix)+file_suffix + \
                  '_frank_uv_resid.npz resid_vis/')
        os.system('mv '+target+'_kink'+str(kink_ix)+file_suffix + \
                  '_frank_profile_fit.txt mprofiles/')
        os.system('rm '+target+'_kink'+str(kink_ix)+file_suffix+'_frank*')

        # record parameter values (F_cpd in uJy, j, r_cpd, az_cpd)
        with open(target+'_kink'+str(kink_ix)+'_mpars.'+subsuf+'.txt', 'a') as f:
            f.write('%i    %s    %.3f    %i\n' % \
...
mpars_name = f"{target}_kink{kink_ix}_mpars.{subsuf}.txt"
