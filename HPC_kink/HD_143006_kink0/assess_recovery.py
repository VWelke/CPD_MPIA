...
# target disk/kink; iteration
target, kink_ix, subsuf = 'HD_143006', '0', '0'
...
rec_file = 'recoveries/'+target+'_kink'+str(kink_ix)+'_recoveries.'+subsuf+'.txt'
...
imdir = f'/nexus/posix0/MIA-astro-env/myben/vawelke/inj_rev/{target}_kink{kink_ix}/resid_images/'
imfile = imdir+target+'_kink'+kink_ix+'.F'+str(int(Fi[0]))+'uJy'+ \
         '_0000.resid.fits'
...
rkink = disk.disk[target]['rkink'][int(kink_ix)]
wkink = disk.disk[target]['wkink'][int(kink_ix)]
...
np.savetxt('recoveries/'+target+'_kink'+str(kink_ix)+'_rprofs.'+subsuf+'.txt',
...
### Flux recovery bias / dispersion in kink region
