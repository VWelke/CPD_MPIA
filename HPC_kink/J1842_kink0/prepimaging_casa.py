...
inj_file = 'injections/'+target+'_kink'+kink_ix+'_mpars.'+subsuf+'.txt'
...
rfile = 'resid_vis/' + target + '_kink' + kink_ix + \
        '_F' + Fstr[0] + 'uJy_' + mstr[0] + '_frank_uv_resid'
resid_suffix = 'kink'+kink_ix+'.F'+Fstr[0]+'uJy_'+mstr[0]+'.resid'
...
exportfits(im_outfile+'.mask',
           target+'_kink'+kink_ix+'.'+subsuf+'.mask.fits', overwrite=True)

# clean up
for ext in ['.image', '.mask', '.model', '.pb', '.residual']:
    os.system('rm -rf '+im_outfile+ext)
os.system('rm -rf data/'+target+'_data.'+resid_suffix+'.ms*')
os.system('rm -rf '+target+'_kink'+kink_ix+'.'+subsuf+'.psf')
os.system('mv '+im_outfile+'.psf '+target+'_kink'+kink_ix+'.'+subsuf+'.psf')
os.system('rm -rf '+target+'_kink'+kink_ix+'.'+subsuf+'.sumwt')
os.system('mv '+im_outfile+'.sumwt '+target+'_kink'+kink_ix+'.'+subsuf+'.sumwt')
